...
- Formation operations (`GetFormation`, `Place`, `SwapSlots`) preserve sparse positions and commit placement plus membership atomically. Campaign snapshots those positions; campaign opponents are current roster characters and shared roster conversion accepts optional wave `slotId`. See `FORMATION_RULES.md` for board and targeting behavior.
...
- `CombatUnitFactory` is the shared conversion path for gameplay and QA. Characters with `statProfileId` use named `stat-sheet-v1` profiles. The 71 `garrett-roster-v1` collectible definitions use explicit authored bases with canonical class growth, owned level and evolution stars. Bundled campaign waves now instantiate those same character definitions on the enemy side; the legacy bundled enemy catalog is empty. `COMBAT_RULES.md` defines indexing, flat star SPD, caps and rounding. Gear and Core scaling remain unimplemented.
...
The validator rejects missing/duplicate IDs, invalid stat/growth values, unsupported targets/effects/status names, missing skill/roster-opponent references, malformed waves/rewards and invalid banner rates/pools. Legacy normalization remains only for compatibility with old fixtures. The 71 imported characters declare class-growth-v1 and garrett-roster-v1; validation checks their explicit identity/stat path and all campaign wave IDs resolve against the roster. SLICE_CONTENT.md records the current source and temporary-kit boundary.
...
Current save schema **5** migrates the earlier five-slot board into twelve hex spaces per side. Old slots 0/1/2/3/4 map to 0/2/8/9/11; older team-order saves follow the same map. Valid schema-5 sparse positions survive reload without remapping. Invalid/duplicate/stale assignments recover deterministically. Existing balances, owned units, progress and storage keys remain intact. Optional campaign-opponent `slotId` accepts current depth/lane names and migrated legacy aliases, remaining compatible with stage schema 1. The serialized field remains named `enemyId`, but garrett-roster-campaign-v2 interprets it as a roster character ID. Waves retain a five-fighter cap despite twelve available spaces.
